...
public class SambaFileUploader {
    ...
    static void uploadFile(MethodCall call, MethodChannel.Result result)  {
        ...
        executor.execute(() -> {
            try {
                SmbFile file = new SmbFile(call.argument("uploadFolder").toString() + call.argument("fileName").toString(), new NtlmPasswordAuthentication(call.argument("domain"), call.argument("username"), call.argument("password")));
